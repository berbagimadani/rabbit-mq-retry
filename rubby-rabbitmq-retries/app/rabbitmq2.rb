...
def start_subscriber(ch)
  @mailman_users_created_q.subscribe(manual_ack: true) do |delivery_info, properties, payload|
	...
	if retry_count < MAX_RETRIES
      putsi "rejecting (retry via DLX) #{delivery_info.delivery_tag}"; ch.reject(delivery_info.delivery_tag)
    else
      putsi "max retries reached - acking #{retry_count}"; ch.ack(delivery_info.delivery_tag)
    end
  end
end
